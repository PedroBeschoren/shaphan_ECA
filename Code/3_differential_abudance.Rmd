...
this sets the pairwise comparisons you want to make
...
# define alternative function for plotting, (no key, labels on small trees):
phyloseq_to_heat_tree2<-function(ps_object, sample_group){
...
#alternatively, the plot above wihtout key and with labels on small trees

# plot 4 matrixes of trees
heat_tree_l_l2<-mapply(function(ps_obj, ASV_l){
  
        heat_tree_dose<-phyloseq_to_heat_tree2(ps_object = prune_taxa(taxa = ASV_l%>%unlist%>%unique(), x = ps_obj),
                                           sample_group = "frass_dose")
...
# arrange the pltos in a 6-pannel for bacteria
heat_tree_pannel_dose2<-  
ggarrange(plotlist = heat_tree_l_l2,
          ncol =2, 
          nrow = 2,
          labels = c("Bacteria, bulk, dose",
                     "Bacteria, rhizo, dose",
...
# Compare No-Frass treatment to source X and Y (good fungi)
For Bacteria, compare N to frass treamment 2 and 5 (the factors that better seaprate it in the NMDS)
For Fungi, compare to frass source X and Y  (the factors that better seaprate it in the NMDS)
...
# Compare No-Frass treatment to all others
...
# make list of deseqed phyloseq objects ; crossing  model of 3 factors
deseq_3add_l <- lapply (unnormalized_BacFunBulkRhizo_ps_l, function (x)
  phyloseq_to_deseq2(x, ~ amendment))

deseq_3add_l <- lapply (deseq_3add_l, function (x) 
  DESeq(x, test="Wald", fitType="parametric"))

# this unlisted, unsplit deseq2 object will be necessary to construct the heatmap
ps_deseq_bac <- phyloseq_to_deseq2(unnormalized_BacFun_ps_l$Bac, ~ amendment)
ps_deseq_bac <- DESeq(ps_deseq_bac, test="Wald", fitType="parametric")

ps_deseq_fun <- phyloseq_to_deseq2(unnormalized_BacFun_ps_l$Fun, ~ amendment)
ps_deseq_fun <- DESeq(ps_deseq_fun, test="Wald", fitType="parametric")
...
# make a list of the pairwise comparisons you want to run according the treatment ; 
#  positive fold change: higher in fist factor (treatment) / negative fold change: higher in second factor(control)
pairwise_comparison<-list(c("amendment", "no_frass", "frass") )
...
#save pltos externally
ggsave(filename = "./Results/heatmap_frass_NoFrass_Bac_Bulk.pdf",
       plot = heatmap_l$Bac_Bulk)
ggsave(filename = "./Results/heatmap_frass_NoFrass_Bac_Rhizo.pdf",
       plot = heatmap_l$Bac_Rhizo)

ggsave(filename = "./Results/heatmap_frass_NoFrass_Fun_Bulk.pdf",
       plot = heatmap_l$Fun_Bulk)
ggsave(filename = "./Results/heatmap_frass_NoFrass_Fun_Rhizo.pdf",
       plot = heatmap_l$Fun_Rhizo)
...
ggarrange(heat_tree_l_l$Bac_Bulk,
          ...
          ncol =2, 
          nrow = 2,
          labels = c("Bacteria, bulk, no frass VS all frass",
                     "Bacteria, rhizo, no frass VS all frass",
                     "Fungi, bulk, no frass VS all frass",
                     "Fungi, rhizo, no frass VS all frass"))


# export the pannels as pdf
ggsave(heat_tree_pannel_dose, filename = "./Results/heattree_BacFun_nofrass_VS_allfrass.pdf",
       width = 40,
       height = 40,
       units = "cm")
