...
plot(sort(c(sigtab_results_bulk[[1]]$padj, sigtab_results_bulk[[2]]$padj, sigtab_results_bulk[[3]]$padj)))
hist(sigtab_results_bulk[[1]]$log2FoldChange, breaks = 50)


# let's decrease the number of ASVs that pass the filter wtha  more restrictive p value
sigtab_results_bulk<-build_sig_tab(deseq_obj = deseq_3add_l$bulk_soil,
                                  contrast_cha_vector = pairwise_comparison,
                                  unsplit_ps = unnormalized_BacFun_ps_l$Bac,
...
ggsave(filename = "./Results/deseq_bulk_bac.pdf",
       plot = heatmp,
       width = 30,
       height = 30,
       units = "cm")
