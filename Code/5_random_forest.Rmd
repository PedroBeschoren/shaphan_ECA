...
imp_ps@tax_table[,2] %>%table%>%sort(decreasing = TRUE)
imp_ps@tax_table[,3] %>%table%>%sort(decreasing = TRUE)
imp_ps@tax_table[,4] %>%table%>%sort(decreasing = TRUE)
imp_ps@tax_table[,5] %>%table%>%sort(decreasing = TRUE)
imp_ps@tax_table[,6] %>%table%>%sort(decreasing = TRUE)
...
# heat tree atemps
```{r}
#exchange species info per ASV ID; remove taxonomy assingment confidence intervals
...
rf_importance_byOTU<-ASV_stats_confirmed$Bac_Bulk

# transform from phyloseq to  taxmap object
rf_important_metacoder<-parse_phyloseq(prune_taxa(taxa = ASV_confirmed_l[[1]], x = CSS_BacFunBulkRhizo_ps_l$Bac_Bulk) )
...
rf_important_metacoder$data$RF_importance_nodes$meanImp[1]<-3.5
...
heat_tree(rf_important_metacoder,
                ...
                node_size_interval = c(min_RF_importance, max_RF_importance), # define node size range according RF importances
                 ...
                 output_file = "./Results/RF_features_bulk_bac.pdf") # Saves the plot as a pdf file
