...
# put fixing, spliting, training and testing all in a single function. this is for a regression rf model
fix_split_train_test_regression<-function (boruta_output_l, ps_object_l, variable_to_be_classified){
  # this fucntion will fix tentative features in a list of boruta objects
  # then it will split a list of phyloseq objects into training and test sets
  # then it will train the list of models
  # then it will test the lsit of models
...
# remove frass-only samples, adjsut column names, fix list
```{r}
...
# define imporatan features with boruta
Boruta_fun_rhizo<-
  Boruta(shoot_biomass~.,   # classification you are trying to predict
         data = single_physeq_to_borutaInput(physeq_object = CSS_BacFunBulkRhizo_ps_l$Fun_Rhizo,
...
boruta_biomass_formula<-fixed_boruta_formula(Boruta_fun_rhizo)

#split train and test sets
train_test_df<-train_and_test_spliter(ps_object = CSS_BacFunBulkRhizo_ps_l$Fun_Rhizo,
...
# train the model
model_borutized_biomass <- caret::train(boruta_biomass_formula, 
                                         data = train_test_df$data_Train, 
                                         method = "rf", 
...
arrange(model_borutized_biomass$results, RMSE) %>% head

# check the predictions of the model on the test set
caret::predict.train(object = model_borutized_biomass,
              newdata = train_test_df$data_Test)

#values of biomass on test set
train_test_df$data_Test$shoot_biomass

#quick plot of predicted VS test set
ggplot(
  data = train_test_df$data_Test,
  aes(x = train_test_df$data_Test$shoot_biomass, y = caret::predict.train(object = model_borutized_biomass,
              newdata = train_test_df$data_Test))
) + geom_point()
...
prediction<-caret::predict.train(object = model_borutized_biomass, newdata = train_test_df$data_Test)

# define the eror between test and training set
error<- prediction - train_test_df$data_Test$shoot_biomass

#calculate Root Mean Square Error
sqrt(mean(error ^ 2))

# plot error (residuals
plot(train_test_df$data_Test$shoot_biomass, error)
