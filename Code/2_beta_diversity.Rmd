...
# rename no frass condition
CSS_BacFun_ps_l<-lapply(CSS_BacFun_ps_l, function(y){
  
    sample_data(y)$frass_source <- gsub(pattern = "N", replacement = "NoFrass", x = sample_data(y)$frass_source)
    
    sample_data(y)$frass_treatment_and_dose <- gsub(pattern = "N_0", replacement = "NoFrass", x = sample_data(y)$frass_treatment_and_dose)
    
    sample_data(y)$frass_treatment_and_dose <- gsub(pattern = "_", replacement = "", x = sample_data(y)$frass_treatment_and_dose)
    
    sample_data(y)$frass_treatment_and_dose <- factor(x = sample_data(y)$frass_treatment_and_dose,
                                                      levels = c("C2", "U2", "C5", "U5", "NoFrass"))
...
# 1 - separate figures and update visuals and legends for 2026 review
```{r}

# calculate ordinations
NMDS_BacFun_l <-lapply(CSS_bacfun_ps_sampletype_l, function (x)
  lapply(x[c(1,3)], function(y)
          ordinate(y,
                   method="NMDS",
                   distance="bray",
                   try=2000,
...
NMDS_calculation_l <- function(physeq_l, ordination_output) {
  ...
  untitled_plot_l<-mapply(function(x, y)
    phyloseq::plot_ordination(physeq = x, 
      ...
      scale_shape_manual(values=c(1,16,2,17,4))+
    #  geom_point(aes(fill = frass_source))+
      ...
      labs(color="Frass source", 
           shape="Frass treatment and dose"),
    x = physeq_l,
    y = ordination_output,
    SIMPLIFY = FALSE)
...
NMDS_Bac_l<-NMDS_calculation_l(physeq_l = CSS_bacfun_ps_sampletype_l$Bac[c(1,3)], 
                               ordination_output = NMDS_BacFun_l$Bac )

NMDS_Bac_l$rhizosphere<-NMDS_Bac_l$rhizosphere+
  ggtitle("Rhizosphere bacteria")
NMDS_Bac_l$bulk_soil<-NMDS_Bac_l$bulk_soil+
  ggtitle("Bulk soil bacteria")


NMDS_Fun_l<-NMDS_calculation_l(physeq_l = CSS_bacfun_ps_sampletype_l$Fun[c(1,3)], 
                               ordination_output = NMDS_BacFun_l$Fun )

NMDS_Fun_l$rhizosphere<-NMDS_Fun_l$rhizosphere+ 
  ggtitle("Rhizosphere fungi")
NMDS_Fun_l$bulk_soil<-NMDS_Fun_l$bulk_soil+
  ggtitle("Bulk soil fungi")

# arrange all 4 plots in a pannel
NMDS_l<-ggarrange(NMDS_Bac_l$bulk_soil,
...
#export plot
ggsave(filename = "./Results/NMDS_Bac_fun_2_sample_types.pdf" ,
       plot = NMDS_l,
       width = 225,
       height = 225,
       units = "mm" )
...
############
# hard code frass-only samples plot and export
###########

# calculate ordinations
NMDS_BacFun_OnlyFrass_l <-lapply(CSS_bacfun_ps_sampletype_l, function (x)
...
# make plots
bac_OnlyFrass<-
    phyloseq::plot_ordination(physeq = CSS_bacfun_ps_sampletype_l$Bac$frass, 
                              ordination = NMDS_BacFun_OnlyFrass_l$Bac$frass,
                              color ="frass_source",
                              shape = "frass_treatment")+
      scale_shape_manual(values=c(1,16,2,17,4))+
      theme_classic() +
      labs(subtitle = paste("Stress", round(NMDS_BacFun_OnlyFrass_l$Bac$frass$stress, digits=4))) +
  ggtitle("K__Bacteria")+
      theme(plot.title = element_text(size = 10, face = "bold")) + 
      theme(legend.position="right")+
      labs(color="Frass source", 
           shape="Frass treatment")
    
# make plots
fun_OnlyFrass<-
    phyloseq::plot_ordination(physeq = CSS_bacfun_ps_sampletype_l$Fun$frass, 
                              ordination = NMDS_BacFun_OnlyFrass_l$Fun$frass,
                              color ="frass_source",
                              shape = "frass_treatment")+
      scale_shape_manual(values=c(1,16,2,17,4))+
      theme_classic() +
      labs(subtitle = paste("Stress", round(NMDS_BacFun_OnlyFrass_l$Fun$frass$stress, digits=4))) +
    ggtitle("K__Fungi")+
      theme(plot.title = element_text(size = 10, face = "bold")) + # extarcts on where this is Bac or Fun, add to plot title
      theme(legend.position="right")+
      labs(color="Frass source", 
...
#export plot
ggsave(filename = "./Results/NMDS_Bac_fun_OnlyFrass.pdf" ,
       plot = Join_frassOnly_plot,
       width = 225,
       height = 110,
       units = "mm" )
...
CSS_bacfun_BulkRhizo_ps_l<-lapply(CSS_bacfun_BulkRhizo_ps_l, function(y)
  lapply(y, function(x){
  x<-subset_samples(x, frass_source != "NoFrass") # from frass source N
...
alpha_plot_nofrass<-lapply(calculated_diversity_df_l, function (x)
  ...
  facet_wrap(~ sample_type,
             labeller = as_labeller(function(x) str_to_title(str_replace_all(x, "bulk_soil", "Bulk Soil"))))+
  labs(title = "Shannon diversity indexes",
       color="Frass source",
       fill ="Frass source",
       alpha = NULL,
       x = "Frass treatment and dose",
...
calculated_diversity_filtred_df_l<-lapply(calculated_diversity_df_l, function (x)
 dplyr::filter(x, frass_source !="NoFrass" & sample_type !="frass"))
