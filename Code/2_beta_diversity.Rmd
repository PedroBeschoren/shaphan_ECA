...
## SUBSET
x <- CSS_bacfun_BulkRhizo_ps_l$Bac$rhizosphere # select Bac/Fun and rhizosphere/bulk soil

x <- subset_samples(x, frass_source=="X" & frass_dose=="5")

dist <- phyloseq::distance(otu_table(x), method="bray")
metadata <- as(sample_data(x), "data.frame")


pairwise.adonis2(dist ~ frass_treatment, data = metadata)
...
pairwise.adonis2(dist ~ frass_treatment_dose_source, data = metadata)
...
x <- calculated_diversity_filtred_df_l$Bac
x <- subset(x, frass_source=="X")

ggplot(x, aes( x =frass_dose, y = Shannon, fill =frass_dose ))+
  geom_violin(aes(alpha = 0.3))+
  geom_dotplot(binaxis = "y", stackdir = "center", position = "dodge", binwidth = 0.05, aes( x =frass_dose))+
  theme_bw()+
  facet_wrap(~ frass_treatment )+
  labs(title = paste("Shannon diversity indexes"))
